submodule(subdomain_m) subdomain_s
  ...
  integer nx, ny, nz, me, num_subdomains, my_internal_left, my_internal_right
...
contains
  ...
  module procedure define
    ...
    self%s_(my_internal_left:my_internal_right,:,1) = boundary_val ! bottom subdomain boundary
    self%s_(my_internal_left:my_internal_right,:,nz) = boundary_val ! top subdomain boundary
    
    self%s_(my_internal_left:my_internal_right, 2:ny-1, 2:nz-1) = internal_val ! internal points

    if (me == 1) then
      self%s_(1, 1:ny, 1:nz) = boundary_val ! left domain boundary
    else
      self%s_(1, 2:ny-1, 2:nz-1) = internal_val ! left subdomain boundary
    end if
    if (me == num_subdomains) then
      self%s_(my_nx, 1:ny, 1:nz) = boundary_val ! right domain boundary
    else
      self%s_(my_nx, 2:ny-1, 2:nz-1) = internal_val ! right subdomain boundary
    end if
  ...
  module procedure laplacian
    ...
    do concurrent(i=my_internal_left+1:my_internal_right-1, j=2:ny-1, k=2:nz-1)
