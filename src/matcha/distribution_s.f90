...
contains
  ...
  module procedure construct
    ...
    call assert(all(sample_distribution2(:,2)>=0.D0), "distribution_t%construct: sample_distribution>=0.", &
      intrinsic_array_t(sample_distribution2))

    associate(nintervals => size(sample_distribution2,1))      
      distribution%vel_ = [(sample_distribution2(i,1), i =1, nintervals)]  ! Assign speeds to each distribution bin         
      distribution%cumulative_distribution_ = [0.D0, [(sum(sample_distribution2(1:i,2)), i=1, nintervals)]]
